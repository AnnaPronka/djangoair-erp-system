...
from djangoair.settings import CURRENCY
import string
import random
import json
...
def first_step(request):
    ...
    if request.method == 'GET':
        data = {}
        data['destination'] = [i['destination__name'] for i in
                               Flight.objects.all().values('destination__name').distinct()]
    ...
    if request.method == 'POST':
        ...
        if not seat:
            ...
            flight_data.ticket_code = hash(
                flight_data.user.email + flight_data.first_name + flight_data.last_name + str(flight_data.flight.id))
            temp = {i: j for i, j in flight_data.__dict__.items() if i not in [
                'destination', 'password', 'currency', 'email', 'username', 'error']}
